...
class RoutingService {
  ...
  async calculateSingleRoute(
    ...
    // Try OSRM first (fastest and most reliable)
    try {
      return await this.calculateWithOSRM(request);
    } catch (error) {
      console.warn('OSRM failed, trying Valhalla:', error);
    }
  ...
  private async calculateWithOSRM(request: RouteRequest): Promise<RouteResponse> {
